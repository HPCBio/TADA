...
colnames(tracking_ddFs) <- c(paste0("dada2.denoised.", opt$stage, ".R1"))
tracking_ddFs <- tracking_ddFs %>%
    as_tibble() %>%
    mutate(SampleID = rownames(tracking_ddFs), .before = 1)
write_csv(tracking_ddFs, paste0("dada2.denoised.", opt$stage, ".R1.csv"))

# generate priors for the next round
...
if (length(dadaRs) > 0) {
    ...
    colnames(tracking_ddRs) <- c(paste0("dada2.denoised.", opt$stage, ".R2"))
    tracking_ddRs <- tracking_ddRs %>%
        as_tibble() %>%
        mutate(SampleID = rownames(tracking_ddRs), .before = 1)
    write_csv(tracking_ddRs, paste0("dada2.denoised.", opt$stage, ".R2.csv"))
...
if (!file.exists(paste0("priors.", opt$stage, ".R1.fna"))) {
    file.create(paste0("priors.", opt$stage, ".R1.fna"))
}
