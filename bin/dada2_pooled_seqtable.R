...
option_list <- list(
    make_option("--readmode", type = "character", help = "Read mode label: R1 or merged"),
    make_option("--rescue_unmerged", type = "logical", default = FALSE,
        help = "Rescue unmerged pairs by concatenating them [default %default]"),
...
rescuePairs <- opt$rescue_unmerged
...
# this is to ensure this acts as if in the dada2 package (so uses any functions defined within)
...
write_csv(seqtab_stats, paste0("seqtab.original.pooled.", opt$readmode, ".csv"))

# this is an optional filtering step to remove *merged* sequences based on
# min/max length criteria
if (opt$min_asv_len > 0) {
    seqtab <- seqtab[, nchar(colnames(seqtab)) >= opt$min_asv_len]
}

if (opt$max_asv_len > 0) {
    seqtab <- seqtab[, nchar(colnames(seqtab)) <= opt$max_asv_len]
}
...
if (opt$min_asv_len > 0 | opt$max_asv_len > 0) {
    ...
    seqtab_stats <- as_tibble_col(seqtab_stats, column_name = "dada2.pooled.seqtab.lengthfiltered") %>%
        mutate(SampleID = nms, .before = 1)
    write_csv(seqtab_stats, paste0("seqtab.", opt$readmode, ".lengthfiltered.csv"))
}
